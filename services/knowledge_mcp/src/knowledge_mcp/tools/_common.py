...
(this module is where it originated in Phase 3, before Project/Task/GitHub/
Calendar MCP copied it verbatim). Kept as a one-line re-export so
`tools/*.py`'s existing `from knowledge_mcp.tools._common import ...`
imports (and this service's own tests) don't need to change.
"""
