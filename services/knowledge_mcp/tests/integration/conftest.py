"""Integration test fixtures for Knowledge MCP.

Reuses `apply_migrations`/`db_session`/`TEST_DATABASE_URL` from
`packages/common/tests/conftest.py` (loaded by file path — not `import
conftest`, to avoid colliding with any other module also named `conftest`
when this suite runs as part of a combined monorepo-wide `pytest` session —
see PROGRESS_REPORT.md Phase 3 "Decisions made") rather than duplicating
migration/session-fixture logic, per this phase's task brief.

Data: seeds `db_test` once per test session via Phase 2's
`scripts/generate_all.py --size small --seed 42` (same mechanism as
`scripts/tests/integration/test_seed_db.py`, imported the same way that
file itself is run — by adding `scripts/` to `sys.path`) so tests exercise
real generated notes/tags/wikilinks/embeddings, not hand-built fixtures.
...
instead of opening a real committed transaction — services under test never
know the difference (they just get an `AsyncSession`), and no test leaves
...
async def seed_small_dataset(apply_migrations: None) -> AsyncIterator[None]:
    ...
    session; truncate before and after (mirrors
    `scripts/tests/integration/test_seed_db.py`'s `_clean_slate` fixture)."""
...
def patched_uow(monkeypatch: pytest.MonkeyPatch, db_session: AsyncSession) -> AsyncSession:
    """Route every `knowledge_mcp.repositories.unit_of_work.unit_of_work()`
    call in the code under test through the rolled-back-at-teardown
    `db_session` for this one test."""
    from knowledge_mcp.repositories import unit_of_work as uow_module
