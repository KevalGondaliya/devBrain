...
class CalendarMCPSettings(BaseSettings):
    """Process-wide settings specific to the Calendar MCP server."""
    ...
    # --- Transport ---
    calendar_mcp_transport: str = Field(
        default="stdio",
        description='"stdio" (local Claude Desktop/Code dev) or "streamable-http"/"sse".',
    )
    calendar_mcp_host: str = Field(default="127.0.0.1")
    calendar_mcp_port: int = Field(default=8005)
    ...
    calendar_mcp_stdio_role: str = Field(default="admin")


@lru_cache(maxsize=1)
def get_calendar_mcp_settings() -> CalendarMCPSettings:
    return CalendarMCPSettings()
