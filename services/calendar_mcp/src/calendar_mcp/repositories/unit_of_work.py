...
`services/calendar_service.py::get_adapter`), and threads the resulting
session through however many adapter/repository calls compose that one
logical operation (e.g. `create_event` = insert + idempotency-key lookup +
audit-log row, atomically). Tests monkeypatch this module's `unit_of_work`
to route through the rolled-back-at-teardown `db_session` fixture instead
of opening a real committed transaction.
"""
