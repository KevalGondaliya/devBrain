"""`github_mcp.auth` — role resolution + enforcement at the tool boundary."""
...
from github_mcp import auth as gh_auth
...
def test_resolve_actor_stdio_defaults_to_configured_role(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("GITHUB_MCP_STDIO_ROLE", raising=False)
    actor_ctx = gh_auth.resolve_actor(None)
    assert actor_ctx.role == Role.ADMIN  # default per GithubMCPSettings
    assert actor_ctx.actor == gh_auth.STDIO_ACTOR_LABEL


def test_resolve_actor_stdio_honors_env_override(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("GITHUB_MCP_STDIO_ROLE", "viewer")
    actor_ctx = gh_auth.resolve_actor(None)
    assert actor_ctx.role == Role.VIEWER


def test_resolve_actor_context_without_request_context_treated_as_stdio() -> None:
    actor_ctx = gh_auth.resolve_actor(RaisingContext())  # type: ignore[arg-type]
    assert actor_ctx.actor == gh_auth.STDIO_ACTOR_LABEL
...
def test_resolve_actor_http_valid_bearer_token(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    actor_ctx = gh_auth.resolve_actor(ctx)  # type: ignore[arg-type]
...
def test_resolve_actor_http_missing_header_raises() -> None:
    ...
    with pytest.raises(UnauthorizedError):
        gh_auth.resolve_actor(ctx)  # type: ignore[arg-type]
...
def test_require_min_role_raises_forbidden_when_insufficient(
    ...
    with pytest.raises(ForbiddenError):
        gh_auth.require_min_role(None, Role.ADMIN)


def test_require_min_role_passes_when_sufficient(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("GITHUB_MCP_STDIO_ROLE", "admin")
    actor_ctx = gh_auth.require_min_role(None, Role.USER)
    assert actor_ctx.role == Role.ADMIN
