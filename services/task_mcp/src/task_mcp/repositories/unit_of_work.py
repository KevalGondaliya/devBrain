...
repository calls compose that one logical operation (e.g. `create_task` =
project-existence check + insert + audit-log row, atomically). Tests
monkeypatch this module's `unit_of_work` to route through the
rolled-back-at-teardown `db_session` fixture instead of opening a real
